...
The patches adapt UTM QEMU's TCG region allocator to external RW/RX mappings, disable APRR switching for the iOS split-alias path, and fix compiler warnings. Patched upstream files retain their original licenses: `tcg/region.c` MIT, `util/error-report.c` GPL-2.0-or-later, `include/tcg/tcg-apple-jit.h` LGPL-2.1-or-later. Full upstream source/licenses are fetched at the pinned revision for a host build; host binaries are not packaged by this repository's CI. The iOS packaging pipeline now requires the QEMU framework and its actual dependency closure. A successful iOS build has not yet been verified in this environment.
