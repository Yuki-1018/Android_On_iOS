...
{
    // Large texture uploads must not pin tens of MiB in every idle connection.
    if (!m_validData && m_size > 1024 * 1024) {
        unsigned char *small = (unsigned char *)realloc(m_buf, 64 * 1024);
        if (small) { m_buf = small; m_size = 64 * 1024; m_readPtr = m_buf; }
    }
    ...
    if (len==0) {
        ...
        if (m_size >= 64u * 1024 * 1024) return -1;
