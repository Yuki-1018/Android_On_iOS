...
- `scripts/angle_metal_image.py` adds the missing `EGL_GL_TEXTURE_2D_KHR` source to the pinned Metal `ImageMtl`, using a retained native mip view exported by `TextureMtl`. Source orphaning detaches its storage while image targets retain the previous pixels. Only `EGL_KHR_gl_texture_2D_image` is added; GL renderbuffer/cubemap/3D image source targets remain unadvertised. This patch requires iOS/macOS ANGLE validation; host Mesa tests check the expected sharing contract, not Metal execution.

- Metal shared-image handoffs now finish producer writes before helper reads and finish helper writes before returning buffer-swap/upload replies. This conservative synchronization can reduce throughput; host GPU tests exercise the same barriers. Failed framebuffer readbacks do not publish stale/uninitialized callback data. Device flicker resolution and frame-time impact still require iOS validation.
